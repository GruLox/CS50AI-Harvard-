...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        neighbors, count = self.getNeighbors(cell, count)
        ...
        for s in self.knowledge:
            if s == sentence:
                ...
                difference = s.cells.difference(sentence.cells)
                
                if s.count == sentence.count:
                    for safeFound in difference:
                        self.mark_safe(safeFound)
                
                elif len(difference) == s.count - sentence.count:
                    for mineFound in difference:
                        self.mark_mine(mineFound)
                
                else:
                    newInferences.append(Sentence(difference, s.count - sentence.count))

            elif sentence.cells.issuperset(s.cells):
                difference = sentence.cells.difference(s.cells)

                if s.count == sentence.count:
                    for safeFound in difference:
                        self.mark_safe(safeFound)

                elif len(difference) == sentence.count - s.count:
                    for mineFound in difference:
                        self.mark_mine(mineFound)

                else:
                    newInferences.append(Sentence(difference, sentence.count - s.count))
                    
        self.knowledge.extend(newInferences)
        self.deleteDuplicates()
        self.deleteKnownsFromKnowledge()
    ...
    def deleteDuplicates(self):
        uniqueKnowledge = []
        for s in self.knowledge:
            if s not in uniqueKnowledge:
                uniqueKnowledge.append(s)

        self.knowledge = uniqueKnowledge

    def deleteKnownsFromKnowledge(self):
        knowledge = []
        for s in self.knowledge:
            knowledge.append(s)
            if s.known_mines():
